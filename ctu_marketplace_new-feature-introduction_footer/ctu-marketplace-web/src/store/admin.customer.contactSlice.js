...
import CustomerContactService from "../services/admin.customer.contact.service";

const GET_CUSTOMER_CONTACTS = 'admin/customer/contact/getAll';
const GET_CUSTOMER_CONTACT = 'admin/customer/contact/getById';
const CREATE_CUSTOMER_CONTACT = 'admin/customer/contact/create';
const UPDATE_CUSTOMER_CONTACT = 'admin/customer/contact/update';
const DELETE_CUSTOMER_CONTACT = 'admin/customer/contact/delete';
const DELETE_ALL_CUSTOMER_CONTACT = 'admin/customer/contact/delete/all';
...
export const retrieveCustomerContactsByUserId = createAsyncThunk(
  GET_CUSTOMER_CONTACTS,
  async (userId) => {
    const res = await CustomerContactService.getAllByUserId(userId);
    return res.data.data;
  }
);
...
export const customerContactSlice = createSlice({
  name: "admin.customer.contact",  // Tên của slice, mỗi slice đặt 1 tên khác nhau để phân biệt
  initialState,
  ...
  extraReducers: {
    ...
    // Get all by user id
    [retrieveCustomerContactsByUserId.pending]: (state, action) => {
      state.isloading = true;
      return state
    },
    [retrieveCustomerContactsByUserId.fulfilled]: (state, action) => {
      state.isloading = false;
      state.data = {...state.data, ..._.mapKeys(action.payload, 'id')}
      return state
    },
    [retrieveCustomerContactsByUserId.rejected]: (state, action) => {
      state.isloading = true;
      return state
    },
